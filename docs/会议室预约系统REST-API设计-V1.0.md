...
公共 `ScheduleBooking`：`id,roomId,subject,organizerName,startTime,endTime,displayStatus`；`displayStatus` 按当前北京时间派生为 `UPCOMING`、`IN_PROGRESS`、`ENDED`，不持久化。公共日程不返回参会人员、说明、取消原因、部门或联系方式，也不显示已取消预约。
...
| `GET /me/bookings?page=&size=` | 已认证 | 分页 | `200 {items,page,size,total,totalPages}`，按 `startTime DESC,id DESC` | `401` |
...
| Method / Path | 权限 | 请求 | 成功响应 | 错误 |
| --- | --- | --- | --- | --- |
| `GET /admin/bookings?page=&size=` | ADMIN | 分页 | `200` 管理员摘要分页，按 `startTime DESC,id DESC` | `401`、`403` |
...
| `GET /admin/bookings/export?fromDate=&toDate=` | ADMIN | 可选日期范围；省略为当天 | `200 text/csv; charset=utf-8` | `400`、`401`、`403` |
