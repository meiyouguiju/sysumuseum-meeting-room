...
public interface BookingAuditLogMapper {
    ...
    @Insert("""
            ...
            """)
    int insertAdminUpdateAudit(AdminBookingUpdateAuditLogEntity auditLog);
    ...
    @Insert("""
            INSERT INTO booking_audit_log (
                ...
                #{bookingId}, 'CANCEL', #{actorUserId}, #{actorRoleSnapshot}, #{targetOwnerUserId}, #{reason},
                #{versionBefore}, #{versionAfter}, CAST(#{beforeJson} AS JSON),
                CAST(#{afterJson} AS JSON), CAST(#{slotChangeJson} AS JSON), #{occurredAt}
            )
            """)
    int insertAdminCancelAudit(AdminBookingCancelAuditLogEntity auditLog);
}
