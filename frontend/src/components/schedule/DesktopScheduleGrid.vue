...
import { buildDaySlots, currentMinutesInShanghai, timeToMinutes, todayInShanghai } from '@/utils/schedule'
...
const gridStyle = computed(() => ({ '--slot-count': slots.value.length, '--room-column-width': `${ROOM_COLUMN_WIDTH}px`, '--slot-width': `${SLOT_WIDTH}px` }))
const currentMinutes = ref(currentMinutesInShanghai())
const showCurrentTimeLine = computed(() => currentMinutes.value >= 0 && props.schedule.date === todayInShanghai())
const currentTimeLineStyle = computed(() => ({ left: `${ROOM_COLUMN_WIDTH + currentMinutes.value / props.schedule.slotMinutes * SLOT_WIDTH}px` }))
const currentTimeLabel = computed(() => `${String(Math.floor(currentMinutes.value / 60)).padStart(2, '0')}:${String(currentMinutes.value % 60).padStart(2, '0')}`)
...
<template>
  ...
  <div v-else ref="scrollContainer" class="schedule-grid-scroll">
    <div class="schedule-grid" :style="gridStyle">
      ...
      <div v-if="showCurrentTimeLine" class="current-time-label" :style="currentTimeLineStyle" aria-hidden="true">{{ currentTimeLabel }}</div>
      <div v-if="showCurrentTimeLine" class="current-time-line" :style="currentTimeLineStyle" aria-hidden="true" />
      <ScheduleRoomRow
...
.schedule-grid { position: relative; min-width: max-content; }
.sticky-time-header { position: sticky; top: 0; z-index: 3; background: #f8fafc; }
.current-time-label { position: absolute; top: 3px; z-index: 4; transform: translateX(-50%); padding: 2px 5px; border-radius: 3px; color: #fff; background: #ef4444; font-size: 11px; font-weight: 700; line-height: 1.2; pointer-events: none; }
.current-time-line { position: absolute; top: 42px; bottom: 0; z-index: 2; border-left: 2px dashed #ef4444; pointer-events: none; }
</style>
