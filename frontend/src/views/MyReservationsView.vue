...
async function handleError(error: unknown, date: string) {
  ...
  if (apiError.errorCode === 'BOOKING_VERSION_CONFLICT') {
    ...
    if (
      ...
      await Promise.all([detailQuery.refetch(), refreshRelated(date, selectedId.value!)])
  }
}
...
async function cancelBooking() {
  ...
  try {
    await ElMessageBox.confirm(
      `${value.subject}\n${value.room.name}\n${value.startTime.slice(0, 10)} ${formatTimeRange(value.startTime, value.endTime)}${extra}`,
      '确定取消该预约吗？',
      ...
      <el-table class="desktop-reservation-table" :data="listQuery.data.value.items">
        ...
        <el-table-column label="操作" width="190"
          ><template #default="{ row }"
            ...
            ><el-button v-if="canEdit(row)" link type="primary" @click="openEdit(row.id)"
              >修改</el-button
            ><el-button v-if="canCancel(row)" link type="danger" @click="openDetail(row.id)"
              >取消</el-button
            ></template
          ></el-table-column
        >
